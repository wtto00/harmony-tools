...
<template>
  <div class="items-center justify-between p-4 text-sm rounded-lg transition-transform-opacity duration-500"
    :class="[colorClass, state.message ? '' : 'opacity-0 -translate-y-1', closed && !state.message ? 'hidden' : 'flex']"
    role="alert" @transitionend="transitionend">
    <svg class="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
      fill="currentColor" viewBox="0 0 24 24">
      <path v-if="state.type === 'info'"
        d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10S2 17.523 2 12Zm9.408-5.5a1 1 0 1 0 0 2h.01a1 1 0 1 0 0-2h-.01ZM10 10a1 1 0 1 0 0 2h1v3h-1a1 1 0 1 0 0 2h4a1 1 0 1 0 0-2h-1v-4a1 1 0 0 0-1-1h-2Z" />
      <path v-else-if="state.type === 'danger'"
        d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10S2 17.523 2 12Zm11-4a1 1 0 1 0-2 0v5a1 1 0 1 0 2 0V8Zm-1 7a1 1 0 1 0 0 2h.01a1 1 0 1 0 0-2H12Z" />
      <path v-else-if="state.type === 'success'"
        d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10S2 17.523 2 12Zm13.707-1.293a1 1 0 0 0-1.414-1.414L11 12.586l-1.793-1.793a1 1 0 0 0-1.414 1.414l2.5 2.5a1 1 0 0 0 1.414 0l4-4Z" />
      <path v-else-if="state.type === 'warning'"
    ...
    <button type="button"
      ...
      :class="closeColorClass" aria-label="Close" @click="close">
